public class WinStrategy extends ComputerStrategy {
	...
	private NotLoseStrategy successor;
	...
	public int[] computeComputerMove(int playerNumber, GameBoard board) {
		...
		for (x = 0; x < 3; x++) {
			xCount = 0;
			for (y = 0; y < 3; y++) {
				if ((newBoard.checkPiece(x, y)) == 2) {
					xCount++;
				} else if ((newBoard.checkPiece(x, y)) == 1) {
					xCount = 0;
					break;
				} else {
					pos[0] = x;
